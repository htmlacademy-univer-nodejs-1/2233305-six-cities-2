...
describe('GET /offers/premium', async () => {
  test('Success get premium offers', async (tc) => {
    const user = {
    ...
    const response = await fetch(new URL(`/offers/premium/${CityEnum.Amsterdam}`, url), {
      method: 'GET',
      headers: new Headers({
        'content-type': 'application/json'
      })
  ...
  test('Success get premium offers empty', async (tc) => {
    const user = {
      email: 'test1@test.com',
      password: '12345678910',
    };
    ...
    const response = await fetch(new URL(`/offers/premium/${CityEnum.Cologne}`, url), {
      method: 'GET',
      headers: new Headers({
        'content-type': 'application/json'
      })
