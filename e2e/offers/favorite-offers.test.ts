...
describe('GET /offers/favorites', async () => {
  test('Success get favorite offers', async (tc) => {
    const user = {
    ...
    const getFavoriteResponse = await fetch(new URL('/offers/users/favorites', url), {
      method: 'GET',
      headers: new Headers({
        'content-type': 'application/json',
        'Authorization': `Bearer ${content.token}`
    ...
    tc.expect(getFavoriteResponse.ok).toBeTruthy();
    tc.expect(getFavoriteResponse.status).toStrictEqual(200);
